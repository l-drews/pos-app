...
export default defineNuxtPlugin({
  ...
  async setup() {
    ...
    if (typeof window !== "undefined" && window.__electronORPC) {
      ...
      // vanish until a manual reload — so supervise it and re-establish a
      // fresh channel on close. In-flight requests on the dead channel are
      // rejected by the link; the next retry/refetch flows over the new one.
